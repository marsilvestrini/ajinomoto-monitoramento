        ...
        try:
            ...
            if detected:
                ...
                if time.time() - self.timeout_start > self.timeout_max:
                    print("[LabelPolpaTracker] Tempo limite excedido sem detectar etiqueta.")
                    self.statusPassoLabelPolpa = False
                    self.alertPassoLabelPolpa = "Etiqueta não identificada na ROI"
                    self.isSpecting = False
                    self.messenger_passos.send_message({"Recolar a UC": self.statusPassoLabelPolpa})
                    self.messenger_alertas.send_message({"alerta": True})
            ...
            now = time.time()
            self.detection_times = [t for t in self.detection_times if now - t <= self.required_time]
            ...
            cv2.putText(frame, "ROI Etiqueta", (x1, y1 - 10), 
